...
unsigned int rgbToRGBE8(const RBVector3 &rgb){
	float v = RBMath::get_max(rgb.x, rgb.y);
	v = RBMath::get_max(v, rgb.z);
...
unsigned int rgbToRGB9E5(const RBVector3 &rgb){
...
const char *getFormatString(const FORMAT format){
	for (unsigned int i = 0; i < ARRAY_COUNT(formatStrings); i++){
...
FORMAT getFormatFromString(char *string){
	for (unsigned int i = 0; i < ARRAY_COUNT(formatStrings); i++){
    if (_stricmp(string, formatStrings[i].string) == 0) return formatStrings[i].format;
  }
  return FORMAT_NONE;
}
...
int Image::getMipMapCountFromDimesions() const {
  int m = RBMath::get_max(width, height);
  m = RBMath::get_max(m, depth);
...
int Image::getMipMappedSize(const int firstMipMapLevel, int nMipMapLevels, FORMAT srcFormat) const {
  ...
  while (nMipMapLevels){
    if (isCompressedFormat(srcFormat)){
      //all the BCn format is compressing per 4x4 block.
      //if the size is not times of 4,make it ceil to 4.
      size += ((w + 3) >> 2) * ((h + 3) >> 2) * d;
    }
    else {
      size += w * h * d;
...
int Image::getPixelCount(const int firstMipMapLevel, int nMipMapLevels) const {
  ...
  //compressed image may be lack of pixels.
  return (depth == 0) ? 6 * size : size;
}
  ...
  if ((str = strstr(header, "-Y ")) != NULL){
    ...
    if ((str = strstr(header, "+X ")) != NULL){
      ...
      if (width < 8 || width > 0x7FFF || buffer[0] != 2 || buffer[1] != 2){
        ...
        pixels = new u8[w * height];
        u8 *src = buffer;

        for (int j = 0; j < height; j++){
          src += 4;

          u8 *sEnd = pixels + (j + 1) * w;
          for (int i = 0; i < 4; i++){
            u8 *sDest = pixels + j * w + i;
            do {
              uint count = *src++;
              if (count > 128){
                count -= 128;
...
bool Image::loadJPEG(const char *fileName){
  ...
  auto handle = g_res_manager->load_resource(fileName);
  ...
  fclose(file);
#endif
  TextureData* tdd = (TextureData*)handle->extra;
  width = tdd->width;
  height = tdd->height;
  depth = 1;
  nMipMaps = 1;
...
bool Image::loadPNG(const char *fileName){
  ...
  if (colorType == PNG_COLOR_TYPE_PALETTE){
    ...
    u8 *newPixels = new u8[width * height * 3];
    if (bitDepth == 4){
  ...
  if (bitDepth == 16){
    // Fix endian
	  int size = width * height * nChannels * sizeof(uint16);
...
#endif // NO_BMP
#ifndef NO_PCX
    else if (_stricmp(extension, ".pcx") == 0){
...
bool Image::loadSlicedImage(const char **fileNames, const int nImages, const int nArraySlices, uint flags){
  ...
  u8 *dest = create(images[0].format, images[0].width, images[0].height, nImages, nMipMaps, nArraySlices);
...
bool Image::saveDDS(const char *fileName){
  ...
  if (isCube()){
    for (int face = 0; face < 6; face++){
      for (int mipMapLevel = 0; mipMapLevel < nMipMaps; mipMapLevel++){
        int faceSize = getMipMappedSize(mipMapLevel, 1) / 6;
        u8 *src = getPixels(mipMapLevel) + face * faceSize;
...
bool Image::saveHDR(const char *fileName){
  ...
  if (format == FORMAT_RGBE8){
    ...
    RBVector3 *src = (RBVector3 *)pixels;
...
bool Image::saveJPEG(const char *fileName, const int quality){
  ...
  return true;
#else
  g_logger->debug_log(WIP_INFO, "Donnt surpport save jpg!No file saved!");
  return false;
#endif
}
#endif // NO_JPEG
...
bool Image::savePNG(const char *fileName){
  ...
  return true;
#else
  g_logger->debug_log(WIP_INFO, "Donnt surpport save png! No file saved!");
  return true;
#endif
}
#endif // NO_PNG
...
bool Image::saveTGA(const char *fileName){
  ...
  if (format == FORMAT_I8){
    ...
    buffer = new u8[height * lineLength];
    int len;
...
bool Image::createMipMaps(const int mipMaps){
  ...
  if (!RBMath::is_pow_2(width) || !RBMath::is_pow_2(height) || !RBMath::is_pow_2(depth)) return false;
  ...
  if (nMipMaps != actualMipMaps){
    ...
    if (arraySize > 1){
      u8 *newPixels = new u8[size * arraySize];
  ...
  for (int arraySlice = 0; arraySlice < arraySize; arraySlice++){
    u8 *src = getPixels(0, arraySlice);
    u8 *dst = getPixels(1, arraySlice);
      ...
      for (int i = 0; i < n; i++){
        if (isPlainFormat(format)){
          if (isFloatFormat(format)){
            ...
            buildMipMap((u16 *)dst, (u16 *)src, w, h, d, nChannels);
          }
          else {
...
bool Image::uncompressImage(){
  if (isCompressedFormat(format)){
    ...
    u8 *newPixels = new u8[getMipMappedSize(0, nMipMaps, destFormat)];

    int level = 0;
    u8 *src, *dst = newPixels;
...
RBVector3 rgbeToRGB(u8 *rgbe){
	if (rgbe[3]){
		return RBVector3(rgbe[0], rgbe[1], rgbe[2]) * ldexpf(1.0f, rgbe[3] - (int)(128 + 8));
	}
	else return RBVector3(0, 0, 0);
}
...
bool Image::unpackImage(){
  ...
  if (format == FORMAT_RGBE8){
    ...
    for (int i = 0; i < pixelCount; i++){
      ((RBVector3 *)newPixels)[i] = rgbeToRGB(pixels + 4 * i);
    }
    ...
    for (int i = 0; i < pixelCount; i++){
      ...
      ((u16 *)newPixels)[4 * i] = (((src)& 0x3FF) * 4198340) >> 16;
	  ((u16 *)newPixels)[4 * i + 1] = (((src >> 10) & 0x3FF) * 4198340) >> 16;
	  ((u16 *)newPixels)[4 * i + 2] = (((src >> 20) & 0x3FF) * 4198340) >> 16;
	  ((u16 *)newPixels)[4 * i + 3] = (((src >> 30) & 0x003) * 21845);
    }
  }
  else {
    return false;
...
bool Image::convert(const FORMAT newFormat){
  ...
  if (format == FORMAT_RGBE8 && (newFormat == FORMAT_RGB32F || newFormat == FORMAT_RGBA32F)){
    newPixels = new u8[getMipMappedSize(0, nMipMaps, newFormat) * arraySize];
    float *dest = (float *)newPixels;

    bool writeAlpha = (newFormat == FORMAT_RGBA32F);
    u8 *src = pixels;
    do {
      *((RBVector3 *)dest) = rgbeToRGB(src);
      if (writeAlpha){
        dest[3] = 1.0f;
        dest += 4;
      }
    ...
    u8 *src = pixels;
    u8 *dest = newPixels = new u8[getMipMappedSize(0, nMipMaps, newFormat) * arraySize];
    ...
    if (format == FORMAT_RGB8 && newFormat == FORMAT_RGBA8){
      ...
      do {
        ...
        if (isFloatFormat(format)){
          ...
          for (int i = 0; i < nSrcChannels; i++) rgba[i] = ((u16 *)src)[i] * (1.0f / 65535.0f);
        }
        else {
        ...
        if (isFloatFormat(newFormat)){
          if (newFormat <= FORMAT_RGBA32F){
            ...
            if (newFormat == FORMAT_RGBE8){
              *(uint32 *)dest = rgbToRGBE8(RBVector3(rgba[0], rgba[1], rgba[2]));
            }
            else {
              *(uint32 *)dest = rgbToRGB9E5(RBVector3(rgba[0], rgba[1], rgba[2]));
            }
          }
        }
        else if (newFormat >= FORMAT_I16 && newFormat <= FORMAT_RGBA16){
          for (int i = 0; i < nDestChannels; i++)	((u16 *)dest)[i] = (u16)(65535 * RBMath::clamp(rgba[i],0.f,1.f) + 0.5f);
        }
        else if (/*isPackedFormat(newFormat)*/newFormat == FORMAT_RGB10A2){
          *(uint *)dest =
			  (uint(1023.0f * RBMath::clamp(rgba[0], 0.f, 1.f) + 0.5f) << 22) |
			  (uint(1023.0f * RBMath::clamp(rgba[1], 0.f, 1.f) + 0.5f) << 12) |
			  (uint(1023.0f * RBMath::clamp(rgba[2], 0.f, 1.f) + 0.5f) << 2) |
			  (uint(3.0f * RBMath::clamp(rgba[3], 0.f, 1.f) + 0.5f));
        }
        else {
          for (int i = 0; i < nDestChannels; i++)	dest[i] = (unsigned char)(255 *RBMath::clamp(rgba[i],0.f,1.f) + 0.5f);
        }
...
bool Image::flipY(){
  ...
  u8 *newPixels = new u8[getMipMappedSize(0, nMipMaps)];
  u8 *dst = newPixels;
  ...
  for (int i = 0; i < nMipMaps; i++){
    ...
    u8 *src = getPixels(i) + (h - 1) * lineWidth;
...
bool Image::toRGBD16(){
  ...
  u16 *newPixels = new u16[nPixels * 4];
  for (uint i = 0; i < nPixels; i++){
    float maxChannel = max(max(max(src[3 * i], src[3 * i + 1]), src[3 * i + 2]), 1.0f);

    newPixels[4 * i + 0] = (u16)(65535 * (src[3 * i + 0] / maxChannel));
    newPixels[4 * i + 1] = (u16)(65535 * (src[3 * i + 1] / maxChannel));
    newPixels[4 * i + 2] = (u16)(65535 * (src[3 * i + 2] / maxChannel));
    newPixels[4 * i + 3] = (u16)(65535 * (1.0f / maxChannel));
  }
...
bool Image::toRGBE16(float &scale, float &bias){
  ...
  for (uint i = 0; i < nPixels; i++){
    ...
    newPixels[4 * i + 1] = (u16)(65535 * src[3 * i + 1] / maxChannel);
    newPixels[4 * i + 2] = (u16)(65535 * src[3 * i + 2] / maxChannel);
    newPixels[4 * i + 3] = (u16)(65535 * RBMath::clamp((chExp - bias) / scale,0.f,1.f));
  }
...
bool Image::toE16(float *scale, float *bias, const bool useAllSameRange, const float minValue, const float maxValue){
  ...
  u16 *newPixels = new u16[nElements];
  for (uint i = 0; i < nElements; i += nRanges){
    for (uint k = 0; k < nRanges; k++){
      if (src[i + k] > minValue){
        if (src[i + k] < maxValue){
          float chExp = logf(src[i + k]) * invLog2;
          newPixels[i + k] = (u16)(65535 * (chExp - bias[k]) / scale[k]);
        }
        else {
          newPixels[i + k] = 65535;
        }
...
bool Image::toFixedPointHDR(float *maxValue, const int finalRgbBits, const int finalRangeBits){
  ...
  u16 *newPixels = new u16[4 * nPixels];
  for (uint i = 0; i < nPixels; i++){
    ...
    */
    ...
    newPixels[4 * i + 0] = (u16)(65535 * r);
    newPixels[4 * i + 1] = (u16)(65535 * g);
    newPixels[4 * i + 2] = (u16)(65535 * b);
    newPixels[4 * i + 3] = (u16)(65535 * range);
  }
...
bool Image::toGrayScale(){
  ...
  if (format <= FORMAT_RGBA8){
    ...
    u16 *src = (u16 *)pixels, *dest = (u16 *)pixels;
    do {
      *dest++ = (77 * src[0] + 151 * src[1] + 28 * src[2] + 128) >> 8;
      src += nChannels;
    } while (--len);
...
bool Image::removeChannels(bool keepCh0, bool keepCh1, bool keepCh2, bool keepCh3){
  ...
  u8 *newPixels = new u8[getMipMappedSize(0, nMipMaps) * arraySize];
