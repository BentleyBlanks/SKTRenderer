...
static std::map<ID3D11SamplerState*, RBD3D11SamplerState*> GSamplerStateCache;


void EmptyD3DSamplerStateCache()
{
	for (auto Iter : GSamplerStateCache)
	{
...
RBSamplerStateRHIRef FD3D11DynamicRHI::RHICreateSamplerState(const RBSamplerStateInitializerRHI& Initializer)
{
	D3D11_SAMPLER_DESC SamplerDesc;
	rb_memzero(&SamplerDesc, sizeof(D3D11_SAMPLER_DESC));
	...
	SamplerDesc.MaxAnisotropy = RBMath::clamp<uint32>(Initializer.MaxAnisotropy,1,16);
	...
	const RBColor32 LinearBorderColor = RBColor32(Initializer.BorderColor);
	...
	auto it = GSamplerStateCache.find(SamplerStateHandle);
	if (it != GSamplerStateCache.end())
	{
		RBD3D11SamplerState* Found = it->second;
		return Found;
	}

	RBD3D11SamplerState* SamplerState = new RBD3D11SamplerState;
	SamplerState->Resource = SamplerStateHandle;
	// Manually add reference as we control the creation/destructions
	SamplerState->AddRef();
	GSamplerStateCache[SamplerStateHandle.GetReference()] =  SamplerState;
	return SamplerState;
}

RBRasterizerStateRHIRef FD3D11DynamicRHI::RHICreateRasterizerState(const RBRasterizerStateInitializerRHI& Initializer)
{
	D3D11_RASTERIZER_DESC RasterizerDesc;
	rb_memzero(&RasterizerDesc, sizeof(D3D11_RASTERIZER_DESC));
	...
	RasterizerDesc.FrontCounterClockwise = false;//ue4 is true
	RasterizerDesc.DepthBias = (i32)(Initializer.DepthBias * (float)(1 << 24));
	RasterizerDesc.DepthClipEnable = true;
	RasterizerDesc.MultisampleEnable = Initializer.bAllowMSAA;
	RasterizerDesc.ScissorEnable = true;

	RBD3D11RasterizerState* RasterizerState = new RBD3D11RasterizerState;
	VERIFYD3D11RESULT(Direct3DDevice->CreateRasterizerState(&RasterizerDesc, RasterizerState->Resource.GetInitReference()));
	return RasterizerState;
}

RBDepthStencilStateRHIRef FD3D11DynamicRHI::RHICreateDepthStencilState(const RBDepthStencilStateInitializerRHI& Initializer)
{
	RBD3D11DepthStencilState* DepthStencilState = new RBD3D11DepthStencilState;

	D3D11_DEPTH_STENCIL_DESC DepthStencilDesc;
	rb_memzero(&DepthStencilDesc, sizeof(D3D11_DEPTH_STENCIL_DESC));
...
RBBlendStateRHIRef FD3D11DynamicRHI::RHICreateBlendState(const RBBlendStateInitializerRHI& Initializer)
{
	D3D11_BLEND_DESC BlendDesc;
	rb_memzero(&BlendDesc, sizeof(D3D11_BLEND_DESC));
	...
	{
		const RBBlendStateInitializerRHI::FRenderTarget& RenderTargetInitializer = Initializer.RenderTargets[RenderTargetIndex];
	...
	RBD3D11BlendState* BlendState = new RBD3D11BlendState;
