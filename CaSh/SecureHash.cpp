...
{
  std::string ToString(const FMD5Hash& Hash)
  {
    if (!Hash.bIsValid)
    {
      return std::string();
  ...
  void FromString(FMD5Hash& Hash, const TCHAR* Buffer)
  {
    ...
    ::memcpy(Hash.Bytes, Bytes, 16);
    Hash.bIsValid = true;
  }
}

FMD5Hash FMD5Hash::HashFile(const TCHAR* InFilename, std::vector<uint8>* Buffer)
{
  ...
  if (Ar)
  {
    ...
    while (Position < Size)
    {
      const auto ReadNum = RBMath::get_min(Size - Position, (int64)Buffer->Num());
