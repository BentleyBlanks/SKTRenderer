...
bool SkinShadingFastApp::preinit()
{
...
bool SkinShadingFastApp::init()
{
  ...
  _sssss_buffer.init_stencil = 1;
  _sssss_buffer.sss_follow_surface = 1;
  _sssss_buffer.sss_fovy = _cam->get_fovy();
  _sssss_buffer.sss_width = 0.1;
  _sssss_buffer.dir = RBVector2(1, 0);

	return true;
}

void SkinShadingFastApp::update(f32 dt)
{
...
void SkinShadingFastApp::draw()
{
  ...
  //lighting pass
	...
	_context->OMSetRenderTargets(1, &_buffer_lighting_target_view, _buffer_depth_stencil_ds_view);
	//_context->ClearDepthStencilView(_buffer_depth_stencil_ds_view, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
	_context->ClearRenderTargetView(_buffer_lighting_target_view, reinterpret_cast<const float*>(&RBColorf::blank));
  ...
  //sssss
	_context->VSSetShader(_def_vertex_shader_light, 0, 0);
	_context->PSSetShader(_post_sssss_shader, 0, 0);

  //_context->PSSetShaderResources(1, 1, &_bufferDepth_shader_view);
  RBMatrix m;
  _cam->get_perspective_matrix(m);
  m = m.get_inverse_slow();
  _sssss_buffer.inv_projection = m;
	...
	_context->IASetVertexBuffers(0, 1, &_vertex_buffer_lighting, &stride, &offset);
	_context->IASetIndexBuffer(_index_buffer_lighting, DXGI_FORMAT_R16_UINT, 0);
	_context->PSSetSamplers(0, 1, &_sample_state_linear);
  _context->PSSetSamplers(1, 1, &_sample_state_point);
  _context->PSSetShaderResources(1, 1, &_bufferDepth_shader_view);

  //horizontal pass
  _sssss_buffer.dir = RBVector2(1,0);
  set_post_sssss_buffer();
	_context->PSSetShaderResources(0, 1, &_buffer_lighting_shader_view);
	_context->OMSetRenderTargets(1, &_ping_pong_buffer_render_view[0], _buffer_depth_stencil_ds_view);
  _context->ClearRenderTargetView(_ping_pong_buffer_render_view[0], reinterpret_cast<const float*>(&RBColorf::black));
	_context->DrawIndexed(6, 0, 0);
  _context->OMSetRenderTargets(0, 0, 0);

  //vertical pass
  _sssss_buffer.dir = RBVector2(0,1);
  set_post_sssss_buffer();
  _context->PSSetShaderResources(0, 1, &_ping_pong_buffer_shader_views[0]);
  _context->OMSetRenderTargets(1, &_ping_pong_buffer_render_view[1], _buffer_depth_stencil_ds_view);
  _context->ClearRenderTargetView(_ping_pong_buffer_render_view[1], reinterpret_cast<const float*>(&RBColorf::black));
  _context->DrawIndexed(6, 0, 0);
  _context->OMSetRenderTargets(0, 0, 0);


  _context->PSSetShader(_post_shader,0,0);
  set_screen_size();
  _context->PSSetSamplers(0, 1, &_sample_state);
  _context->PSSetShaderResources(0, 1, &_ping_pong_buffer_shader_views[1]);
...
void SkinShadingFastApp::ter()
{
	FreeLibrary(CompilerDLL);
	CompilerDLL = 0;
}

void SkinShadingFastApp::postter()
{
	g_res_manager->shutdown();
}

SkinShadingFastApp::SkinShadingFastApp(AppBase* a) :FastApp(a)
{

}

SkinShadingFastApp::~SkinShadingFastApp()
{

}

void SkinShadingFastApp::load_assets()
{
	//g_res_manager->load_resource("shaders/d.vs", WIPResourceType::TEXT);
	load_shaders();
	load_deffered_pipe();
	/** load models */
	load_models_generate_buffer("Res/head/head.obj");
}



void SkinShadingFastApp::show_error_message(ID3D10Blob* error, const char* filename)
{
...
void SkinShadingFastApp::load_models_generate_buffer(const char* filename)
{
	...
	/** load file */
	{
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			//memset(_vecs.data(), 0, sizeof(SkinShadingFastApp::Vertex_PNTT)*numvertex);
  ...
  //diffuse
  {
	  auto ret = create_texture_from_file("res/head/diffuse.jpg", &_texture1, &_texture1_sview);
	  if (!ret)
		  printf("diffuse wrong!!\n");
  }
  //normal
  {
    
	  auto ret = create_texture_from_file("res/head/normal.jpg", &_normal_texture, &_normal_texture_shader_view);
	  if (!ret)
		  printf("normal wrong!!\n");
      
    
    /*
    const wchar_t* fileName = m2u("res/head/normal.dds");
  ...
  //power
  {
	  auto ret = create_texture_from_file("res/head/spec.jpg", &_power_texture, &_power_texture_shader_view);
	  if (!ret)
		  printf("spec wrong!!\n");
  }
  //opacity
...
void SkinShadingFastApp::set_matrix(const RBMatrix& m, const RBMatrix& v, const RBMatrix& p)
{
	HRESULT hr;
...
void SkinShadingFastApp::handle_input(f32 dt)
{
	...
	{
		_high_light -= 0.01f;
		_high_light = RBMath::clamp(_high_light, 0.53f, 1.f);
		printf("%f\n", _high_light);

	}
  if (Input::get_key_pressed(WIP_O))
  {
    _sssss_buffer.cam_range += 0.001;
    _sssss_buffer.cam_range = RBMath::clamp(_sssss_buffer.cam_range, 0.f, 10000.f);
    printf("%f\n",_sssss_buffer.cam_range);
  }
  if (Input::get_key_pressed(WIP_P))
  {
    _sssss_buffer.cam_range -= 0.001;
    _sssss_buffer.cam_range = RBMath::clamp(_sssss_buffer.cam_range, 0.f, 10000.f);
    printf("%f\n", _sssss_buffer.cam_range);
  ...
  {
    _sssss_buffer.sss_width += 0.1;
    _sssss_buffer.sss_width = RBMath::clamp(_sssss_buffer.sss_width, 0.f, 190.f);
    printf("%f\n", _sssss_buffer.sss_width);
  }
  if (Input::get_key_pressed(WIP_T))
  {
    _sssss_buffer.sss_width -= 0.1;
...
void SkinShadingFastApp::set_screen_size()
{
	HRESULT hr;
	D3D11_MAPPED_SUBRESOURCE mpr;
...
void SkinShadingFastApp::set_gpass_vbuffer()
{
	HRESULT hr;
	D3D11_MAPPED_SUBRESOURCE mpr;
	...
	if (FAILED(hr))
	{
		::MessageBox(0, "Map post buffer failed!", "ERROR", MB_OK);
	}
...
void SkinShadingFastApp::set_gbuffer_hair_p_cbuffer()
{
	HRESULT hr;
	D3D11_MAPPED_SUBRESOURCE mpr;

	hr = _context->Map(_def_gbuffer_p_hair_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mpr);
	if (FAILED(hr))
	{
		::MessageBox(0, "Map _def_gbuffer_p_hair_buffer buffer failed!", "ERROR", MB_OK);
	}

	GHairPBuffer* mb;
	mb = (GHairPBuffer*)mpr.pData;
	D3D11_TEXTURE2D_DESC des;
	_normal_texture->GetDesc(&des);
	mb->w = des.Width;
	mb->h = des.Height;
	_context->Unmap(_def_gbuffer_p_hair_buffer, 0);
	_context->PSSetConstantBuffers(0, 1, &_def_gbuffer_p_hair_buffer);
}

void SkinShadingFastApp::set_post_buffer(RBVector2 dir)
{
	HRESULT hr;
	D3D11_MAPPED_SUBRESOURCE mpr;

	hr = _context->Map(_post_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mpr);
	if (FAILED(hr))
	{
		::MessageBox(0, "Map screen size buffer failed!", "ERROR", MB_OK);
	}
	...
	PostBuffer* mb;
	mb = (PostBuffer*)mpr.pData;
	mb->step_dir = dir;
	_context->Unmap(_post_buffer, 0);
	_context->VSSetConstantBuffers(1, 1, &_post_buffer);

}

void SkinShadingFastApp::load_shaders()
{
	/** load shaders */
	{
		...
		if (compile_shader)
		{
			...
			else
			{
				_device->CreatePixelShader(Shader->GetBufferPointer(), Shader->GetBufferSize(), NULL, &_def_pixel_shader);
				{
					...
					mbd.Usage = D3D11_USAGE_DYNAMIC;
					mbd.ByteWidth = PAD16(sizeof(GHairPBuffer));
					mbd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
					mbd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
					mbd.MiscFlags = 0;
					mbd.StructureByteStride = 0;
					HR(_device->CreateBuffer(&mbd, 0, &_def_gbuffer_p_hair_buffer));
				}
			}
			g_res_manager->free(res_handle1, res_handle1->size);
		}
	...
	{
		auto res_handle1 = g_res_manager->load_resource("shaders/deferrend_skin_shading_p.hlsl", WIPResourceType::TEXT);
		TRefCountPtr<ID3DBlob> Shader;
		TRefCountPtr<ID3DBlob> Errors;
		...
		if (compile_shader)
		{
			...
			Result = compile_shader(
				((std::string*)res_handle1->ptr)->data(),
				((std::string*)res_handle1->ptr)->length(),
				"shaders/deferrend_skin_shading_p.hlsl",
				/*pDefines=*/ NULL,
				/*pInclude=*/ NULL,
				"PsMain",
				"ps_5_0",
			...
			if (FAILED(Result))
			{
				if (Errors)
				{
					show_error_message(Errors, "shaders/deferrend_skin_shading_p.hlsl");
				}
				assert(0);
			}
			else
	...
	{
		auto res_handle1 = g_res_manager->load_resource("shaders/post_p.hlsl", WIPResourceType::TEXT);
		TRefCountPtr<ID3DBlob> Shader;
		TRefCountPtr<ID3DBlob> Errors;
		...
		if (compile_shader)
		{
			...
			if (FAILED(Result))
			{
				if (Errors)
				{
					show_error_message(Errors, "shaders/post_p.hlsl");
				}
				assert(0);
			}
			else
			{
				_device->CreatePixelShader(Shader->GetBufferPointer(), Shader->GetBufferSize(), NULL, &_post_shader);
				/** constant buffer */
				{
					D3D11_BUFFER_DESC mbd;
				...
				}
				{
					...
					mbd.Usage = D3D11_USAGE_DYNAMIC;
					mbd.ByteWidth = PAD16(sizeof(PostBuffer));
					mbd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
					mbd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
					mbd.MiscFlags = 0;
					mbd.StructureByteStride = 0;
					HR(_device->CreateBuffer(&mbd, 0, &_post_buffer));
				}
			}
			g_res_manager->free(res_handle1, res_handle1->size);
		}
  ...
  {
    auto res_handle1 = g_res_manager->load_resource("shaders/sssss.hlsl", WIPResourceType::TEXT);
    TRefCountPtr<ID3DBlob> Shader;
    TRefCountPtr<ID3DBlob> Errors;
    ...
    if (compile_shader)
    {
      ...
      if (FAILED(Result))
      {
        if (Errors)
        {
          show_error_message(Errors, "shaders/sssss.hlsl");
        }
        assert(0);
      }
      else
      {
        _device->CreatePixelShader(Shader->GetBufferPointer(), Shader->GetBufferSize(), NULL, &_post_sssss_shader);
        /** constant buffer */
        {
          D3D11_BUFFER_DESC mbd;

          mbd.Usage = D3D11_USAGE_DYNAMIC;
          mbd.ByteWidth = PAD16(sizeof(SSSSSBuffer));
          mbd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
          mbd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
          mbd.MiscFlags = 0;
          mbd.StructureByteStride = 0;
          HR(_device->CreateBuffer(&mbd, 0, &_post_sssss_cbuffer));
        }
      }
      g_res_manager->free(res_handle1, res_handle1->size);
    }
...
void SkinShadingFastApp::load_deffered_pipe()
{
  ...
  //HDR
  {
	  ...
	  D3D11_RENDER_TARGET_VIEW_DESC rtdes;
	  rtdes.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
	  rtdes.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
	  rtdes.Texture2D.MipSlice = 0;
	  HR(_device->CreateRenderTargetView(_buffer_lighting, &rtdes, &_buffer_lighting_target_view));
	  D3D11_SHADER_RESOURCE_VIEW_DESC srdes;
	  memset(&srdes, 0, sizeof(srdes));
	  srdes.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
	  srdes.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
	  srdes.Texture2D.MostDetailedMip = 0;
	  srdes.Texture2D.MipLevels = 1;
  ...
  }
    {
      for (int i = 0; i < 2; ++i)
      {
        ...
        desc.CPUAccessFlags = 0;
        desc.MiscFlags = 0;
        HR(_device->CreateTexture2D(&desc, NULL, _ping_pong_buffers + i));
        D3D11_RENDER_TARGET_VIEW_DESC rtdes;
        rtdes.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
        rtdes.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
        rtdes.Texture2D.MipSlice = 0;
        HR(_device->CreateRenderTargetView(_ping_pong_buffers[i], &rtdes, _ping_pong_buffer_render_view + i));
        D3D11_SHADER_RESOURCE_VIEW_DESC srdes;
        memset(&srdes, 0, sizeof(srdes));
        srdes.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
        srdes.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
        srdes.Texture2D.MostDetailedMip = 0;
        srdes.Texture2D.MipLevels = 1;
        HR(_device->CreateShaderResourceView(_ping_pong_buffers[i], &srdes, _ping_pong_buffer_shader_views + i));
      }
    }
  {
	  D3D11_TEXTURE2D_DESC desc;
...
void SkinShadingFastApp::create_rhires()
{
	{
  ...
  {
    D3D11_SAMPLER_DESC spdes;
    spdes.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
    spdes.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
    spdes.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;
    spdes.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
    spdes.MipLODBias = 0.0f;
    spdes.MaxAnisotropy = 0;
    spdes.ComparisonFunc = D3D11_COMPARISON_ALWAYS;
    spdes.BorderColor[0] = 0;
    spdes.BorderColor[1] = 0;
    spdes.BorderColor[2] = 0;
    ...
    HR(_device->CreateSamplerState(&spdes, &_sample_state_linear));
  }
    {
      D3D11_SAMPLER_DESC spdes;
      spdes.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
      ...
      HR(_device->CreateSamplerState(&spdes, &_sample_state_point));
    }
...
bool SkinShadingFastApp::create_texture_from_file(const char* filename, ID3D11Texture2D** tex, ID3D11ShaderResourceView** tex_view)
{
...
void SkinShadingFastApp::set_post_sssss_buffer()
{
  HRESULT hr;
  D3D11_MAPPED_SUBRESOURCE mpr;

  hr = _context->Map(_post_sssss_cbuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mpr);
  if (FAILED(hr))
  {
    ::MessageBox(0, "Map _post_sssss_cbuffer failed!", "ERROR", MB_OK);
  }

  SSSSSBuffer* mb;
  mb = (SSSSSBuffer*)mpr.pData;
  memcpy(mb,&_sssss_buffer,sizeof(SSSSSBuffer));
  _context->Unmap(_post_sssss_cbuffer, 0);
  _context->PSSetConstantBuffers(0, 1, &_post_sssss_cbuffer);
}
