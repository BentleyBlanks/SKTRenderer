...
class SkinShadingFastApp : public FastApp
{
public:
  ...
  struct SSSSSBuffer
  {
	...
	SkinShadingFastApp(class AppBase* app);
	virtual ~SkinShadingFastApp();
	SkinShadingFastApp(const SkinShadingFastApp& o) = delete;
	SkinShadingFastApp& operator=(const SkinShadingFastApp& o) = delete;
...
protected:
	...
	void set_gbuffer_hair_p_cbuffer();
	void set_post_buffer(RBVector2 dir);
  void set_post_sssss_buffer();
...
private:
	...
	ID3D11Texture2D* _buffer_post;
	ID3D11ShaderResourceView* _buffer_post_target_view;
	ID3D11ShaderResourceView* _buffer_post_shader_view;
  ...
  ID3D11Texture2D* _ping_pong_buffers[2];
  ID3D11ShaderResourceView* _ping_pong_buffer_shader_views[2];
  ID3D11RenderTargetView* _ping_pong_buffer_render_view[2];
	...
	ID3D11PixelShader* _post_shader;
  ID3D11PixelShader* _post_sssss_shader;
	...
	ID3D11Buffer* _post_buffer;
	ID3D11Buffer* _def_gbuffer_p_hair_buffer;
  ID3D11Buffer* _post_sssss_cbuffer;
	ID3D11InputLayout* _layout;
	ID3D11InputLayout* _layout_lighting;
	ID3D11SamplerState* _sample_state;
  ID3D11SamplerState* _sample_state_linear;
  ID3D11SamplerState* _sample_state_point;
