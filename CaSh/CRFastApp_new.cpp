...
void CRFastApp_new::draw()
{
  ...
  _d3d11_renderer->setTexture("diffuse_tex",_color_tex);
  ...
  //_d3d11_renderer->context->VSSetShader(0,0,0);
  //_d3d11_renderer->context->GSSetShader(0, 0, 0);
  //_d3d11_renderer->context->PSSetShader(0, 0, 0);

}

bool CRFastApp_new::init()
{
  ...
  _cam->set_move_speed(RBVector2(10,10));
...
void CRFastApp_new::load_assets()
{
#ifndef COMPLEBYHANDE
  ...
  auto sh2 = D3D11ShaderCompiler::compile(((FD3D11DynamicRHI*)_rhi)->GetDevice(), "shaders/conservation_rasterization_new_g.hlsl", "main", "gs_5_0");
  auto sh3 = D3D11ShaderCompiler::compile(((FD3D11DynamicRHI*)_rhi)->GetDevice(), "shaders/conservation_rasterization_new_p.hlsl", "main", "ps_5_0");
...
void CRFastApp_new::load_models_generate_buffer(const char* filename)
{
	/** load file */
	{
...
void CRFastApp_new::create_rhires()
{
  ...
  _color_tex = _d3d11_renderer->addTexture(im1);
  im1.free();

}

void CRFastApp_new::update(f32 dt)
{
  handle_input(dt);
}

void CRFastApp_new::ter()
{
  _d3d11_renderer->finish();
	delete _d3d11_renderer;
}

void CRFastApp_new::handle_input(f32 dt)
{
  if (Input::get_key_pressed(WIP_Q))
  {
    dx--;
    printf("%d", dx);
    f32 n = _cam->get_near_panel();
    _cam->set_near_panel(n+0.00005);
  }
...
void CRFastApp_new::set_screen_size()
{
...
void CRFastApp_new::set_matrix(const RBMatrix& m, const RBMatrix& v, const RBMatrix& p)
{
...
CRFastApp_new::CRFastApp_new(AppBase* app) :FastApp(app){}

void CRFastApp_new::postter(){}

CRFastApp_new::~CRFastApp_new(){}