...
protected:
  ...
  typedef void(*TSetIndexBufferAlternate)(RBD3D11StateCacheBase* StateCache, ID3D11Buffer* IndexBuffer, DXGI_FORMAT Format, uint32 Offset);
  ...
  typedef void(*TSetSRVAlternate)(RBD3D11StateCacheBase* StateCache, ID3D11ShaderResourceView* SRV, uint32 ResourceIndex, ESRV_Type SrvType);
  template <EShaderFrequency ShaderFrequency>
  ...
  typedef void(*TSetStreamSourceAlternate)(RBD3D11StateCacheBase* StateCache, ID3D11Buffer* VertexBuffer, uint32 StreamIndex, uint32 Stride, uint32 Offset);
  ...
  typedef void(*TSetSamplerStateAlternate)(RBD3D11StateCacheBase* StateCache, ID3D11SamplerState* SamplerState, uint32 SamplerIndex);
  template <EShaderFrequency ShaderFrequency>
...
public:
...
#if D3D11_ALLOW_STATE_CACHE
    D3D11_STATE_CACHE_VERIFY_PRE();
    if ((CurrentNumberOfViewports != 1 || memcmp(&CurrentViewport[0], &Viewport, sizeof(D3D11_VIEWPORT))) || GD3D11SkipStateCaching)
    {
...
#if D3D11_ALLOW_STATE_CACHE
    D3D11_STATE_CACHE_VERIFY_PRE();
    if ((CurrentNumberOfViewports != Count || memcmp(&CurrentViewport[0], Viewports, sizeof(D3D11_VIEWPORT) * Count)) || GD3D11SkipStateCaching)
    {
      ::memcpy(&CurrentViewport[0], Viewports, sizeof(D3D11_VIEWPORT) * Count);
  ...
  D3D11_STATE_CACHE_INLINE void GetViewports(uint32* Count, D3D11_VIEWPORT *Viewports)
  {
    ...
    if (Viewports) //NULL is legal if you just want count
    {
      ...
      int32 CopyCount = RBMath::get_min(RBMath::get_min(StorageSizeCount, (int32)CurrentNumberOfViewports), D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE);
      if (CopyCount > 0)
      {
...
#if D3D11_ALLOW_STATE_CACHE
    D3D11_STATE_CACHE_VERIFY_PRE();
    if ((CurrentBlendState != State || CurrentBlendSampleMask != SampleMask || ::memcmp(CurrentBlendFactor, BlendFactor, sizeof(CurrentBlendFactor))) || GD3D11SkipStateCaching)
    {
...
public:
  ...
  RBD3D11StateCacheBase()
    : Direct3DDeviceIMContext(nullptr)
  {
#if D3D11_ALLOW_STATE_CACHE
    rb_memzero(CurrentShaderResourceViews, sizeof(CurrentShaderResourceViews));
#endif
  }
...
typedef RBD3D11StateCacheBase RBD3D11StateCache;